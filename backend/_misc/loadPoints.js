...
app.get('/points', async function (req, res) {
  try {
    res.send(
      (async () => {
        const locationdata = await csv().fromFile(
          'Kansas County Officials - data.csv'
        )
        const access_token =
        ...
        for (i = 1; i < locationdata.length; i++) {
          fetch(
            `https://api.mapbox.com/geocoding/v5/mapbox.places/${locationdata[i].field5}.json?access_token=${access_token}`
          )
