./csv2pg.py ./states/newmexico.csv import_newmexico

UPDATE import_newmexico
...
SELECT *
FROM import_newmexico
WHERE county NOT IN (
	SELECT j.name
	FROM jurisdiction j
	INNER JOIN state s ON j.state_id = s.id
	WHERE s.abbreviation = 'NM'
)
;
...
INNER JOIN  (
	...
	FROM jurisdiction j
	INNER JOIN state s ON j.state_id = s.id
	LEFT OUTER JOIN import_newmexico imp ON j.name = imp.county
	WHERE s.abbreviation = 'NM'
	GROUP BY j.id
...
FROM jurisdiction j
INNER JOIN state s ON j.state_id = s.id
INNER JOIN import_newmexico imp ON j.name = imp.county
...
SELECT cwj.wip_jurisdiction_id, ft.id, j.name || ' Clerk', imp.address_1, imp.address_2, imp.city, imp.state_2, imp.zip, 'newmexico' tempstring, 'description', 'Unknown', null
FROM jurisdiction j
INNER JOIN state s ON j.state_id = s.id
INNER JOIN import_newmexico imp ON j.name = imp.county
INNER JOIN jurisdictions_with_currwip cwj ON j.id = cwj.jurisdiction_id
