./csv2pg.py ./states/newyork.csv import_newyork

UPDATE import_newyork
...
FROM jurisdiction j
INNER JOIN state s ON j.state_id = s.id
LEFT OUTER JOIN import_newyork imp ON j.name = imp.county
WHERE s.abbreviation = 'NY'
GROUP BY j.id
...
FROM jurisdiction j
INNER JOIN state s ON j.state_id = s.id
INNER JOIN import_newyork imp ON j.name = imp.county
...
SELECT cwj.wip_jurisdiction_id, ft.id, j.name || ' ' || imp.location_name, imp.address_1, imp.address_2, imp.city, imp.state_2, imp.zip, 'newyork-office' tempstring, 'description', 'Unknown'
FROM jurisdiction j
INNER JOIN state s ON j.state_id = s.id
INNER JOIN import_newyork imp ON j.name = imp.county
INNER JOIN jurisdictions_with_currwip cwj ON j.id = cwj.jurisdiction_id
...
FROM (
	...
	WHERE s.abbreviation = 'NY'
	AND u.email = 'drew@anullvalue.net'
	AND (imp.pobox IS NULL OR trim(imp.pobox) = '')
) t0
WHERE wip_jurisdiction.id = t0.wip_jurisdiction_id
;
...
FROM (
	...
	WHERE s.abbreviation = 'NY'
	AND u.email = 'drew@anullvalue.net'
	AND imp.pobox IS NOT NULL AND trim(imp.pobox) <> ''
) t0
WHERE wip_jurisdiction.id = t0.wip_jurisdiction_id
;
