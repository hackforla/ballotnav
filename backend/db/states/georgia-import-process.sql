...
UPDATE import_georgia 
SET county = 'McDuffie County'
WHERE county = 'Mcduffie County'
;

UPDATE import_georgia 
SET county = 'McIntosh County'
WHERE county = 'Mcintosh County'
;
...
FROM jurisdiction j
INNER JOIN state s ON j.state_id = s.id
LEFT OUTER JOIN import_georgia imp ON j.name = imp.county
WHERE s.abbreviation = 'GA'
...
FROM jurisdiction j
INNER JOIN state s ON j.state_id = s.id
INNER JOIN import_georgia imp ON j.name = imp.county
...
SELECT cwj.wip_jurisdiction_id, ft.id, j.name || ' ' || imp.location_name, initcap(imp.address_1), initcap(imp.address_2), initcap(imp.city), imp.state_2, imp.zip, 'georgia-registrars' tempstring, 'description', 'Unknown'
FROM jurisdiction j
INNER JOIN state s ON j.state_id = s.id
INNER JOIN import_georgia imp ON j.name = imp.county
INNER JOIN jurisdictions_with_currwip cwj ON j.id = cwj.jurisdiction_id
