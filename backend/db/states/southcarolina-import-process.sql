./csv2pg.py ./states/southcarolina.csv import_southcarolina

UPDATE import_southcarolina
...
SELECT *
FROM import_southcarolina
WHERE county NOT IN (
	SELECT j.name
	FROM jurisdiction j
	INNER JOIN state s ON j.state_id = s.id
	WHERE s.abbreviation = 'SC'
)
;
...
FROM jurisdiction j
INNER JOIN state s ON j.state_id = s.id
LEFT OUTER JOIN import_southcarolina imp ON j.name = imp.county
WHERE s.abbreviation = 'SC'
GROUP BY j.id
...
FROM jurisdiction j
INNER JOIN state s ON j.state_id = s.id
INNER JOIN import_southcarolina imp ON j.name = imp.county
...
FROM public.import_southcarolina
...
SELECT cwj.wip_jurisdiction_id, ft.id, j.name || ' ' || imp.location_name, imp.address_1, imp.address_2, imp.city, imp.state_2, imp.zip, 'southcarolina-office' tempstring, 'description', imp.hours
FROM jurisdiction j
INNER JOIN state s ON j.state_id = s.id
INNER JOIN import_southcarolina imp ON j.name = imp.county
INNER JOIN jurisdictions_with_currwip cwj ON j.id = cwj.jurisdiction_id
...
FROM (
	SELECT cwj.wip_jurisdiction_id, imp.address_1, imp.address_2, imp.city, imp.state_2, imp.zip, imp.mail_address1, imp.mail_zip
	FROM jurisdiction j
	INNER JOIN state s ON j.state_id = s.id
	INNER JOIN import_southcarolina imp ON j.name = imp.county
	INNER JOIN jurisdictions_with_currwip cwj ON j.id = cwj.jurisdiction_id
	...
	WHERE s.abbreviation = 'SC'
	AND u.email = 'drew@anullvalue.net'
	AND (imp.mail_address1 IS NULL OR trim(imp.mail_address1) = '')
) t0
WHERE wip_jurisdiction.id = t0.wip_jurisdiction_id
;
...
mail_address2 = null,
mail_address3 = (t0.city || ', ' || t0.state_2 || ' ' || t0.mail_zip)
FROM (
	SELECT cwj.wip_jurisdiction_id, imp.address_1, imp.address_2, imp.city, imp.state_2, imp.zip, imp.mail_address1, imp.mail_zip
	FROM jurisdiction j
	INNER JOIN state s ON j.state_id = s.id
	...
	WHERE s.abbreviation = 'SC'
	AND u.email = 'drew@anullvalue.net'
	AND imp.mail_address1 IS NOT NULL AND trim(imp.mail_address1) <> ''
) t0
WHERE wip_jurisdiction.id = t0.wip_jurisdiction_id
;
...
DROP TABLE import_southcarolina;
