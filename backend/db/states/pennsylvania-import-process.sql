./csv2pg.py ./states/pennsylvania.csv import_pennsylvania

UPDATE import_pennsylvania
SET county = trim(county) || ' County'
WHERE county NOT LIKE '%County';

UPDATE import_pennsylvania
SET county = 'Huntingdon County'
WHERE county = 'Huntington County';

-- if you get any rows here, that means the "county" value is not appropriate
SELECT *
FROM import_pennsylvania
WHERE county NOT IN (
	SELECT j.name
	FROM jurisdiction j
	INNER JOIN state s ON j.state_id = s.id
	WHERE s.abbreviation = 'PA'
)
;
...
INNER JOIN  (
	...
	FROM jurisdiction j
	INNER JOIN state s ON j.state_id = s.id
	LEFT OUTER JOIN import_pennsylvania imp ON j.name = imp.county
	WHERE s.abbreviation = 'PA'
	GROUP BY j.id
...
FROM jurisdiction j
INNER JOIN state s ON j.state_id = s.id
INNER JOIN import_pennsylvania imp ON j.name = imp.county
...
SELECT DISTINCT cwj.wip_jurisdiction_id, urlt.id, CASE WHEN (left(imp.county_website_url,4)<>'http') THEN 
...
FROM public.import_pennsylvania
...
SELECT cwj.wip_jurisdiction_id, ft.id, imp.location_name || ' (' || j.name || ')', imp.address_1, imp.address_2, imp.city, imp.state_2, imp.zip, 'pennsylvania' tempstring, 'description', imp.hours, imp.notes
FROM jurisdiction j
INNER JOIN state s ON j.state_id = s.id
INNER JOIN import_pennsylvania imp ON j.name = imp.county
INNER JOIN jurisdictions_with_currwip cwj ON j.id = cwj.jurisdiction_id
...
DROP TABLE import_pennsylvania;
