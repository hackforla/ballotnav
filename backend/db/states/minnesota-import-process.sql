./csv2pg.py ./states/minnesota.csv import_minnesota

UPDATE import_minnesota
SET county = 'St. Louis County'
WHERE county = 'Saint Louis County'
;
...
SELECT *
FROM import_minnesota
WHERE county NOT IN (
	SELECT j.name
	FROM jurisdiction j
	INNER JOIN state s ON j.state_id = s.id
	WHERE s.abbreviation = 'MN'
)
;
...
FROM jurisdiction j
INNER JOIN state s ON j.state_id = s.id
LEFT OUTER JOIN import_minnesota imp ON j.name = imp.county
WHERE s.abbreviation = 'MN'
GROUP BY j.id
...
FROM jurisdiction j
INNER JOIN state s ON j.state_id = s.id
INNER JOIN import_minnesota imp ON j.name = imp.county
...
SELECT cwj.wip_jurisdiction_id, ft.id, imp.location_name || ' (' || j.name || ')', imp.address_1, imp.address_2, imp.city, imp.state_2, imp.zip, 'minnesota-office' tempstring, 'description', imp.hours
FROM jurisdiction j
INNER JOIN state s ON j.state_id = s.id
INNER JOIN import_minnesota imp ON j.name = imp.county
INNER JOIN jurisdictions_with_currwip cwj ON j.id = cwj.jurisdiction_id
