./csv2pg.py ./states/california.csv import_california

UPDATE import_california
SET county = trim(county) || ' County'
WHERE county NOT LIKE '%County';

ALTER TABLE import_california
...
FROM jurisdiction j
INNER JOIN state s ON j.state_id = s.id
LEFT OUTER JOIN import_california imp ON j.name = imp.county
WHERE s.abbreviation = 'CA'
GROUP BY j.id
...
FROM jurisdiction j
INNER JOIN state s ON j.state_id = s.id
INNER JOIN import_california imp ON j.name = imp.county
...
SELECT cwj.wip_jurisdiction_id, ft.id, imp.county, imp.address_1, imp.address_2, imp.city, imp.state_2, imp.zip, 'california-office' tempstring, 'hours'
FROM jurisdiction j
INNER JOIN state s ON j.state_id = s.id
INNER JOIN import_california imp ON j.name = imp.county
INNER JOIN jurisdictions_with_currwip cwj ON j.id = cwj.jurisdiction_id
...
WHERE s.abbreviation = 'CA'
AND u.email = 'drew@anullvalue.net'
AND wl.tempstring = 'california-office'
;
