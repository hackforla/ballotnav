./csv2pg.py ./states/idaho.csv import_idaho
...
INNER JOIN  (
	...
	FROM jurisdiction j
	INNER JOIN state s ON j.state_id = s.id
	LEFT OUTER JOIN import_idaho imp ON j.name = imp.county
	WHERE s.abbreviation = 'ID'
	GROUP BY j.id
...
FROM jurisdiction j
INNER JOIN state s ON j.state_id = s.id
INNER JOIN import_idaho imp ON j.name = imp.county
...
SELECT cwj.wip_jurisdiction_id, ft.id, imp.location_name, imp.address_1, imp.address_2, imp.city, imp.state_2, imp.zip, 'idaho' tempstring, 'description', 'Unknown', null
FROM jurisdiction j
INNER JOIN state s ON j.state_id = s.id
INNER JOIN import_idaho imp ON j.name = imp.county
INNER JOIN jurisdictions_with_currwip cwj ON j.id = cwj.jurisdiction_id
