...
FROM jurisdiction j
INNER JOIN state s ON j.state_id = s.id
LEFT OUTER JOIN import_nebraska imp ON j.name = imp.county
WHERE s.abbreviation = 'NE'
GROUP BY j.id
...
FROM jurisdiction j
INNER JOIN state s ON j.state_id = s.id
INNER JOIN import_nebraska imp ON j.name = imp.county
...
SELECT cwj.wip_jurisdiction_id, ft.id, j.name || ' ' || imp.location_name, imp.address_1, imp.address_2, imp.city, imp.state_2, imp.zip, 'nebraska-auditor-office' tempstring, 'description', 'Unknown'
FROM jurisdiction j
INNER JOIN state s ON j.state_id = s.id
INNER JOIN import_nebraska imp ON j.name = imp.county
INNER JOIN jurisdictions_with_currwip cwj ON j.id = cwj.jurisdiction_id
...
FROM (
	...
	WHERE s.abbreviation = 'NE'
	AND u.email = 'drew@anullvalue.net'
	AND (pobox IS NULL OR trim(pobox) = '')
) t0
