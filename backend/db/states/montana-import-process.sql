...
FROM jurisdiction j
INNER JOIN state s ON j.state_id = s.id
LEFT OUTER JOIN import_montana imp ON j.name = imp.county
WHERE s.abbreviation = 'MT'
...
FROM (
	...
	FROM jurisdiction j
	INNER JOIN state s ON j.state_id = s.id
	INNER JOIN import_montana imp ON j.name = imp.county
...
SELECT cwj.wip_jurisdiction_id, ft.id, j.name, initcap(imp.address_1), initcap(imp.address_2), initcap(imp.city), imp.state_2, imp.zip, 'montana' tempstring, 'description', imp.hours, imp.notes
FROM jurisdiction j
INNER JOIN state s ON j.state_id = s.id
INNER JOIN import_montana imp ON j.name = imp.county
INNER JOIN jurisdictions_with_currwip cwj ON j.id = cwj.jurisdiction_id
