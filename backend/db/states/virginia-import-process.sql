./csv2pg.py ./states/virginia.csv import_virginia

UPDATE import_virginia
SET county = initcap(trim(county));

UPDATE import_virginia
SET county = 'King and Queen County'
WHERE county = 'King & Queen County';

UPDATE import_virginia
SET county = 'Isle of Wight County'
WHERE county = 'Isle Of Wight County';
...
SELECT *
FROM import_virginia
WHERE county NOT IN (
	SELECT j.name
	FROM jurisdiction j
	INNER JOIN state s ON j.state_id = s.id
	WHERE s.abbreviation = 'VA'
)
;
...
FROM jurisdiction j
INNER JOIN state s ON j.state_id = s.id
LEFT OUTER JOIN import_virginia imp ON j.name = imp.county
WHERE s.abbreviation = 'VA'
GROUP BY j.id
...
FROM jurisdiction j
INNER JOIN state s ON j.state_id = s.id
INNER JOIN import_virginia imp ON j.name = imp.county
...
SELECT cwj.wip_jurisdiction_id, ft.id, imp.location_name || ' (' || j.name || ')', imp.address_1, imp.address_2, imp.city, imp.state_2, imp.zip, 'virginia-office' tempstring, 'hours'
FROM jurisdiction j
INNER JOIN state s ON j.state_id = s.id
INNER JOIN import_virginia imp ON j.name = imp.county
INNER JOIN jurisdictions_with_currwip cwj ON j.id = cwj.jurisdiction_id
...
WHERE s.abbreviation = 'VA'
AND u.email = 'drew@anullvalue.net'
AND wl.tempstring = 'virginia-office'
;
