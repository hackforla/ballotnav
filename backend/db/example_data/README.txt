...
Then start up your favorite query processor. Execute the remainder of this file:

BEGIN TRANSACTION;

SELECT generate_calendar('2020-01-01', 365);
...
UPDATE public.scrub_location SET latitude = null WHERE latitude = 'NULL';
UPDATE public.scrub_location SET longitude = null WHERE longitude = 'NULL';
...
SELECT j.id, location_name, info, ft.id, timezone, address_1, address_2, address_3, contact_fax, contact_phone, latitude::numeric, longitude::numeric, 'census_bureau_geocode'
, is_early_dropoff_location::enum_ynu
, is_early_voting_location::enum_ynu
, is_elections_office::enum_ynu
, is_polling_location::enum_ynu
, is_drop_box::enum_ynu
, is_handicap_accessible::enum_ynu
, is_staffed_location::enum_ynu
...
INSERT INTO state_phone (state_id, phonenumbertype_id, phone_number, description, sort_order)
SELECT (SELECT id FROM state WHERE abbreviation='OH'), (SELECT id FROM phonenumbertype WHERE name='Voice'), '+16145551212', '', 1
UNION
SELECT (SELECT id FROM state WHERE abbreviation='OH'), (SELECT id FROM phonenumbertype WHERE name='Voice'), '+18005551212', 'Toll-free in Ohio', 1
UNION
SELECT (SELECT id FROM state WHERE abbreviation='OH'), (SELECT id FROM phonenumbertype WHERE name='TTY'), '+174099941414', '', 1
;
...
SELECT j.id, (SELECT id FROM phonenumbertype WHERE name='Voice'), '+16145253100', '', 1, now(), now()
FROM jurisdiction j
INNER JOIN state s ON j.state_id = s.id
WHERE s.abbreviation = 'OH'
AND j.name = 'Franklin County'
UNION
SELECT j.id, (SELECT id FROM phonenumbertype WHERE name='Voice'), '+16145253489', '', 1, now(), now()
FROM jurisdiction j
INNER JOIN state s ON j.state_id = s.id
WHERE s.abbreviation = 'OH'
AND j.name = 'Franklin County'
...
INSERT INTO public.user (first_name, last_name, email, role, password_hash)
VALUES ('First','Last','nobody@anywhere.tld','admin','');
...
SELECT wip_jurisdiction_create(
(SELECT j.id
FROM jurisdiction j
INNER JOIN state s ON j.state_id = s.id
WHERE j.name = 'Franklin County'
AND s.name = 'Ohio'
)
...
INSERT INTO public.user_jurisdiction(user_id, jurisdiction_id, status)
SELECT (SELECT max(id) FROM public.user), j.id, 'editor'
FROM jurisdiction j
INNER JOIN state s ON j.state_id = s.id
WHERE s.name = 'Ohio'
;
